...
function SwapPage({ swapContract }) {
  ...
  // 将货币金额转换为wei
  const toTokenUnits = (amount, tokenAddress) => {
    const token = tokens.find(t => t.address === tokenAddress);
    if (!token) return ethers.parseEther(amount.toString()); // 默认使用18位小数
    
    return ethers.parseUnits(amount.toString(), token.decimals);
  };

  // 将wei转换为货币金额
  const fromTokenUnits = (units, tokenAddress) => {
    const token = tokens.find(t => t.address === tokenAddress);
    if (!token) return ethers.formatEther(units); // 默认使用18位小数
    
    return ethers.formatUnits(units, token.decimals);
  };
  ...
  useEffect(() => {
    const calculateOutput = async () => {
      if (amountIn > 0 && tokenIn && tokenOut) {
        try {
          const rate = await getExchangeRate(tokenIn, tokenOut);
          setExchangeRate(rate);
          setAmountOut(amountIn * rate);
        } catch (error) {
  ...
  const handleSwap = async () => {
    ...
    try {
      ...
      // 转换输入金额为最小单位
      const amountInUnits = toTokenUnits(amountIn, tokenIn);

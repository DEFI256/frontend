  ...
  const handleAddLiquidity = async () => {
    if (amountA > 0 && amountB > 0) {
      ...
      try {
        ...
        // 将金额转换为wei
        const amountAInWei = toWei(amountA);
        const amountBInWei = toWei(amountB);
        ...
        await contract.methods.addLiquidity(tokenA, amountAInWei, tokenB, amountBInWei).send({ from: account });
